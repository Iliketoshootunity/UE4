...
struct SIosPlatformThread : public SGenericPlatformThread
{
private:
	pthread_t       thread_id;
...
public:
	...
	FORCEINLINE bool start()
	{
		...
		if (status == new_created
			&& !pthread_create(&thread_id, &th_attr, &SIosPlatformThread::ThreadProc, this)) {
			status = running;
			return true;
		}
		return false;
...
typedef SIosPlatformThread SThread;