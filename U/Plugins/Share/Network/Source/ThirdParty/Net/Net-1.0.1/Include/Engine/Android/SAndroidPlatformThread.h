...
struct SAndroidPlatformThread : public SGenericPlatformThread
{
private:
	pthread_t       thread_id;
...
public:
	...
	virtual ~SAndroidPlatformThread()
	{
	...
	FORCEINLINE bool start()
	{
		...
		if (status == new_created
			&& !pthread_create(&thread_id, &th_attr, &SAndroidPlatformThread::ThreadProc, this)) {
			status = running;
			return true;
		}
		return false;
	...
	FORCEINLINE void WaitNextFrame(uint32 ulUSTime = TICK_DELAY_TIME)
	{
		...
		int32 frame_interval = ulUSTime * 1000; // in us
		int32 time_remain_us;
...
typedef SAndroidPlatformThread SThread;